...
first. Simply install serverless, set up the AWS CLI and run  the registries that host the app's docker images. Note
that the I implemented a lifecycle policy that only retains the 10 latest images. I don't want to blow up my AWS storage
bills.
...
### (Optional) Local Installation and Deployment
...
4. Run (optional) `cd <project root>/services/notejam-express && npm install` to install dependencies for the express
app. This requires you to be running nodejs version 6.7.0 to working correctly.
5. (optional) Create a virtualenv using python2.7 as base and run
`cd <project root>/services/notejam-flask && pip install -r requirements.txt` to install dependencies for the flask app.
6. Run the various scripts in `<project root>/package.json` such as `deploy-ecr`, `deploy-db` or `deploy-app` to deploy
or update the corresponding services. Note: set environment variables similar to that in quick start step 4 for the
context in which you would like to deploy the services. If you do not set the variables, deployment may fail or perform
unexpectedly.
...
and no significant progress was made. Will probably try another time to get the right configuration if opportunity
permits.
